...
coinpunk.controllers.Tx.prototype.details = function(txHash) {
  var self = this;
  $.post('/bitlox_web_wallet/api/tx/details', {txHashes: [txHash]}, function(resp) {
...
//           $('#diagnostics').val() = rawtx;
//   $('#diagnostics').innerHTML = rawtx;
  ...
  self.saveWallet({override: true, address: changeAddress}, function(response) {
    if(response.result == 'error' && response.messages[0] == 'Invalid session key') {
      ...
      $.post('/bitlox_web_wallet/api/tx/send', {tx: rawtx}, function(resp) {
        if(resp.error) {
